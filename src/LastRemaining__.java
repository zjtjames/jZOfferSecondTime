...
/**
 * 孩子们的游戏(圆圈中最后剩下的数)：最关键一步 实现从队尾到队首
 *
 ...
 *
 * 思路：用LinkedList模拟游戏过程即可 用链表是考虑在List中间删除节点的成本
 * 算出每次要删除的结点索引号 每次要加上上一次的索引号
 */
public class LastRemaining__ {
    public int lastRemaining_Solution(int n, int m) {
        ...
        int index = 0;
        while (list.size() > 1) {
            index = (index + m - 1) % list.size(); // 算出每次要删除的结点索引号 每次要加上上一次的索引号
            list.remove(index);
        }
